...
LidaImage *IntensityTransformation::histogram_specification_rgb(LidaImage *input)
{
    LidaImage* output = new LidaImage(input->row, input->col);
    int histogram_Red[256];
    int histogram_Green[256];
...
//    inFile.close();
    ...
    *image = image->convertToFormat(QImage::Format_RGB888);
    ...
    LidaImage* input_tmp= IntensityTransformation::QImage2LidaImage(image);
    float histogramSpecified_Red[256];
    float histogramSpecified_Green[256];
    float histogramSpecified_Blue[256];
    for (int i = 0; i < 256; i++) {
        histogramSpecified_Red[i] = 0;
        histogramSpecified_Green[i] = 0;
        histogramSpecified_Blue[i] = 0;
    }
    for (int x = 0; x < input_tmp->row; x++) {
        for (int y = 0; y < input_tmp->col; y++) {
            histogramSpecified_Red[int(input_tmp->matrix3D[0][x][y])]++;
            histogramSpecified_Green[int(input_tmp->matrix3D[1][x][y])]++;
            histogramSpecified_Blue[int(input_tmp->matrix3D[2][x][y])]++;
        }
    }
    int MN = (input_tmp->row) * (input_tmp->col);
    for (int k = 0; k < 256; k++) {
        histogramSpecified_Red[k] = histogramSpecified_Red[k]/MN;
        histogramSpecified_Green[k] = histogramSpecified_Green[k]/MN;
        histogramSpecified_Blue[k] = histogramSpecified_Blue[k]/MN;
    }
    delete image;
    delete input_tmp;
    ...
    for (int k = 0; k < 256; k++) {
        int difference = 256;
        for (int i = 0; i < 256; i++) {
            if(abs(k-sG_Blue[i]) < difference){
                sG_Inverse_Blue[k] = i;
                difference = abs(k-sG_Blue[i]);
            }
        }
    }
    for (int x = 0; x < output->row; x++) {
    ...
    for (int x = 0; x < output->row; x++) {
        for (int y = 0; y < output->col; y++) {
            int r = sG_Inverse_Red[s_Red[int(input->matrix3D[0][x][y])]];
            int g = sG_Inverse_Green[s_Green[int(input->matrix3D[1][x][y])]];
            int b = sG_Inverse_Blue[s_Blue[int(input->matrix3D[2][x][y])]];
            output->matrix3D[0][x][y] = r;
            output->matrix3D[1][x][y] = g;
    ...
    //Output the 3 mapping curve and output histogram statistics
    QString savepath = QFileDialog::getSaveFileName(NULL, "Save mapping function and histogram statistics", NULL, "Excel (*.csv)");
    ofstream outFile;
    outFile.open((savepath.toStdString()).c_str());
    for (int i = 0; i < 256; i++) {
        float R = 0;
        float G = 0;
        float B = 0;
        R = sG_Inverse_Red[s_Red[i]];
        G = sG_Inverse_Green[s_Green[i]];
        B = sG_Inverse_Blue[s_Blue[i]];
//        outFile << R << "," << G << "," << B << endl;
        outFile << R << "," << G << "," << B << "," << "," << ","
                << histogram_Red[i] << "," << histogram_Green[i] << "," << histogram_Blue[i] << "," << "," << ","
                << histogramSpecified_Red[i]*MN << "," << histogramSpecified_Green[i]*MN << "," << histogramSpecified_Blue[i]*MN << "," << "," << ","
                << endl;
    }
    outFile.close();
...
LidaImage *IntensityTransformation::QImage2LidaImage(QImage *input)
{
